...
`;
    ...
    for (let step of steps) {
        ...
        const isMatch = step.selectors.some(sel => {
            ...
            try {
                // Extract just the class names from the selector and compare
                const classMatches = (sel.match(/\.([a-zA-Z0-9_-]+)/g) || [])
                    .map(c => c.slice(1)) // strip leading dot
                    .filter(c => c.length > 0);

                if (classMatches.length > 0) {
                    // Match if target has ANY of the selector's classes
                    if (classMatches.some(cls => targetClasses.includes(cls))) return true;
                }

                // Also try normal selector match as fallback
